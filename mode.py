...
def train(args):
    ...
    while pre_epoch < args.pre_train_epoch:
        ...
        if pre_epoch % 800 == 0:  # 800
            torch.save(generator.state_dict(),
                       'C:/Users/Olek/Desktop/SRGAN2/models/pre_trained_model_%03d.pt' % pre_epoch)
    ...
    while fine_epoch < args.fine_train_epoch:
        ...
        if fine_epoch % 500 == 0:  # 500
            torch.save(generator.state_dict(),
                       'C:/Users/Olek/Desktop/SRGAN2/models/SRGAN_gene_%03d.pt' % fine_epoch)
            torch.save(discriminator.state_dict(),
                       'C:/Users/Olek/Desktop/SRGAN2/models/SRGAN_discrim_%03d.pt' % fine_epoch)
...
def test(args):
    ...
    generator.load_state_dict(torch.load(args.generator_path, map_location=torch.device('cpu')))  ###added ('cpu')
    generator = generator.to(device)
    generator.eval()
    ...
    with torch.no_grad():
        for i, te_data in enumerate(loader):
            ...
            ssim = structural_similarity(y_output / 255.0, y_gt / 255.0, data_range=1.0, channel_axis=2)
            psnr_list.append(psnr)
            ssim_list.append(ssim)
            f.write('psnr : {psnr}, ssim : {ssim}\n'.format(psnr=psnr, ssim=ssim))

            result = Image.fromarray((output * 255.0).astype(np.uint8))
            result.save('./result/res_%04d.png' % i)

        f.write('avg psnr : %04f\n' % np.mean(psnr_list))
        f.write('avg ssim : %04f\n' % np.mean(ssim_list))

        workbook = xlsxwriter.Workbook('lSRGAN.xlsx')
...
def test_only(args):
    ...
    generator = Generator(img_feat=3, n_feats=64, kernel_size=3, num_block=args.res_num)  # 3
    ...
    with torch.no_grad():
        for i, te_data in enumerate(loader):
            ...
            output = output[0].cpu().numpy()
            output = (output + 1.0) / 2.0
            output = output.transpose(1, 2, 0)  # 1 2 0
            result = Image.fromarray((output * 255.0).astype(np.uint8))
            result.save(args.GT_path + '/res_%04d.png' % i)
